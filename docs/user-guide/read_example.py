...
def make_instrument(
    ...
    sample.set_parameters(
        radius=source.xwidth / 2, yheight=source.focus_yh, reflections='"Cu.laz"'
    )
    ...
    if square:
        ...
        monitor.options = (
            f'"mantid square x bins={xbins} y bins={ybins}, '
            f'neutron pixel min={pixel_min} t, l, list all neutrons"'
        )
        ...
        monitor.options = (
            f'"mantid square x bins={xbins} y bins={ybins}, '
            f'neutron pixel min={pixel_min} t, list all neutrons"'
        )
    ...
    if banana:
        ...
        monitor.options = (
            f'"mantid banana theta bins={xbins} limits=[-10, 25] '
        ...
        monitor.set_parameters(
            radius=1.5,
            yheight=0.25,
            filename='"scattered_event_banana_signal.dat"',
            restore_neutron=1,
        )
        monitor.options = (
            f'"mantid banana theta bins={xbins} limits=[-50, 50] '
            f'y bins={ybins}, neutron pixel min={pixel_min} t, list all neutrons"'
        )
